...
# input data files
dependent_filename = 'd12lip.txt'
indicator_filename = 'sr_g_ori_fs.txt'
#######
...
regr_res       = leading_filter(data_lag = g_lag, dependent, sample, p_value)
...
if(save_output){
  ...
  write.zoo(weight, file = paste(output_directory, weight_filename, sep='/'), sep=',', col.names=TRUE, na='')
  write.zoo(mean, file = paste(output_directory, mean_filename, sep='/'), sep=',', col.names=TRUE, na='')
}

# things to do:
# mean and sd for each normalization
# check long run weight matrix starting from 0
# long run: insig table -> a true/false table
# make better UI and dependencies
